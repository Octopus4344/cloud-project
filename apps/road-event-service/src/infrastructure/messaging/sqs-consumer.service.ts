...
export class SqsConsumerService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    private readonly userDataHandler: UserDataProvidedHandler,
    private readonly userLocationHandler: UserLocationProvidedHandler,
  ) {
  ...
  private async poll(): Promise<void> {
    const queueUrl = this.config.get<string>('SQS_ROAD_EVENT_QUEUE_URL')!;
  ...
  private async dispatch(pattern: string, data: unknown): Promise<void> {
    switch (pattern) {
      case 'user.data.provided':
        await this.userDataHandler.handle(data as UserDataProvidedEvent);
        break;
      case 'user.location.provided':
        await this.userLocationHandler.handle(
          data as LocationDataProvidedEvent,
        );
        break;
      default:
